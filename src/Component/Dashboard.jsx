...
function Dashboard() {
    ...
    let dataLoad = async () => {
        setloading(true)
        let book = await axios.get("https://62ab049e371180affbdf40f1.mockapi.io/Libstu")
    ...
    let DeleteBook = async (id) => {
        try {
            ...
            if (ask) {
                await axios.delete(`https://62ab049e371180affbdf40f1.mockapi.io/Libstu/${id}`)
            }
                                            ...
                                            <div className="col mb-5" key={index}>
                                                <div className="card h-100">
                                                    ...
                                                    <div class="btn-group me-2" role="group" aria-label="Second group">
                                                        <button type="button" class="btn btn-primary m-1" onClick={() => { ViewBook(data) }} disabled={!data.Availability}>View</button>
                                                        <button type="button" class="btn btn-primary m-1" onClick={() => { EditBook(data) }} >Edit</button>
                                                        <button type="button" class="btn btn-primary m-1" onClick={() => { DeleteBook(data.id) }} disabled={!data.Availability}>Delete</button>
                                                    </div>
                                                </div>
                                            </div>
                                        </>
